...
## Итерация 17: Финальная серверная полировка для демонстрации

На этом этапе сервер доведен ближе к состоянию, которое удобно показывать на защите диплома и подключать к будущему React-клиенту.

Что добавлено:

- человекочитаемый номер заказа `orderNumber` в формате `TM-2026-000001`;
- поле `adminComment` в заказе;
- таблица `OrderStatusHistory` для истории жизненного цикла заказа;
- сохранение первой записи истории при создании заказа;
- сохранение записи истории при каждом изменении статуса администратором;
- возврат остатков товаров на склад при переводе заказа в `CANCELLED`;
- запрет перевода отмененного заказа обратно в рабочие статусы;
- Postman collection для ручной проверки API;
- frontend API contract для будущего клиента.

Зачем нужен `orderNumber`:

```text
id нужен базе данных и API.
orderNumber нужен человеку: пользователю, оператору и администратору.
```
...
```text
id: cmabc123...
orderNumber: TM-2026-000001
```

История статусов хранит:
...
Это позволяет показать в личном кабинете или админке путь заказа:

```text
NEW -> PROCESSING -> CONFIRMED -> SHIPPED -> COMPLETED
```

Если администратор отменяет заказ:

```text
PATCH /api/admin/orders/:id/status
{
  "status": "CANCELLED",
  "adminComment": "Покупатель отказался от заказа"
}
```

Сервер:

1. создает запись в `OrderStatusHistory`;
2. меняет статус заказа;
3. возвращает товары из заказа на склад;
4. создает уведомление пользователю.

Новые файлы:

```text
docs/frontend-api-contract.md
docs/TechMarket.postman_collection.json
```

`frontend-api-contract.md` нужен как договор между backend и frontend: какие endpoints вызывать, какие токены передавать, какие поля ждать в ответе.

`TechMarket.postman_collection.json` можно импортировать в Postman и показать сценарии:
...
- checkout;
- уведомления;
- админская смена статуса заказа.

Для диплома можно написать:

> Для повышения удобства сопровождения заказов в системе добавлена история изменения статусов. Каждый заказ получает человекочитаемый номер, который можно использовать в интерфейсе и коммуникации с пользователем. При изменении статуса администратором сервер сохраняет запись истории и создает уведомление для пользователя. При отмене заказа система возвращает товары на склад, что сохраняет корректность складских остатков.
